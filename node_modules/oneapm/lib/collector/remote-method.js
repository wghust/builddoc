...
var path = require('path'),
    ...
    .child({
        component: 'remote_method_invoke'
    }),
 ...
 * the callback (which is obviously awesome). To prevent OneAPM from
...
RemoteMethod.prototype._safeRequest = function _safeRequest(options) {
    ...
    var sender = this;
    lookup(options.host, function cb_lookup(error) {
    ...
    if (this._config.feature_flag && this._config.feature_flag.proxy) {
        ...
        if (isProxy) {
            ...
            if (this._config.proxy) {
                proxy_url = url.parse(this._config.proxy);
            } else {
                var proxy_auth = this._config.proxy_user;
                if (this._config.proxy_pass !== '')
                    proxy_auth += ':' + this._config.proxy_pass;
                proxy_url = {
                    protocol: 'http',
                    hostname: this._config.proxy_host || 'localhost',
                    port: this._config.proxy_port,
                    auth: proxy_auth
                }

            }
            requestOptions.host = proxy_url.hostname;
            requestOptions.port = proxy_url.port;
            requestOptions.headers[PROXY_AUTH_HEADER] = 'Basic ' + new Buffer(proxy_url.auth).toString('base64');
...
RemoteMethod.prototype._path = function _path() {
    ...
    if (isProxy) {
        var port_proxy = (this._config.port===443?80:this._config.port)
        return 'http://' + this._config.host + ':' + port_proxy + formatted;
    } else {
        return formatted;
    }
};
