...
try {
  ...
  if (process.version && process.version.split('.')[1] < 6) {
    message = "OneAPM for Node.js requires a version of Node equal to or\n" +
   ...
   * the environment variable ONEAPM_NO_CONFIG_FILE isn't set.
   */
  ...
  if (!config.agent_enabled) {
     ...
     * multiple times, with undefined results. OneAPM's instrumentation
    ...
    if (appNames.length < 1) {
      message = "OneAPM requires that you name this application!\n" +
                "Set app_name in your oneapm.js file or set environment variable\n" +
                "ONEAPM_APP_NAME. Not starting!";
      logger.error(message);
      throw new Error(message);
    }
    ...
    appNames.forEach(function (name) {
      ...
      if (!testVal){
        message = "OneAPM requires that you name this application using alphanumeric" +
              " and certain punctuation characters ([](){}.?!') only.\n" +
              "Reset app_name to follow these naming conventions " +
              "in your oneapm.js file or set environment variable\n" +
              "ONEAPM_APP_NAME. Not starting!";
    ...
    agent.start(function cb_start(error) {
      if (!error) return logger.debug("OneAPM for Node.js is connected to OneAPM.");

      var message = "OneAPM for Node.js halted startup due to an error:";
  ...
  var message = "OneAPM for Node.js was unable to bootstrap itself due to an error:";
  logger.error(error, message);
