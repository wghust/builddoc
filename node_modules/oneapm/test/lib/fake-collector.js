...
var validators = {
  ...
  httpHeaders : function (request, validation) {
    ...
    // OneAPM-NodeAgent/0.9.1-46 (nodejs 0.8.12 darwin-x64)
    var userAgentPattern = /^OneAPM-[a-zA-Z0-9]+\/[0-9.\-]+ \(.+\)$/;
    if (!userAgentPattern.test(request.header('User-Agent'))) {
      validation.header_errors.push("'User-Agent' should conform to OneAPM standards");
    }
...
function bootstrap(options, callback) {
  ...
  server.post('/tpm/agent.do', function (req, res, next) {
    var validations = {};
