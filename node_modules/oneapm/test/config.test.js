...
describe("the agent configuration", function () {
  ...
  function () {
    it("should pick up the application name", function () {
      idempotentEnv('ONEAPM_APP_NAME', 'feeling testy,and schizophrenic',
                    function (tc) {
    ...
    it("should trim spaces from multiple application names ", function () {
      idempotentEnv('ONEAPM_APP_NAME', 'zero,one, two,  three,   four',
                    function (tc) {
        should.exist(tc.app_name);
    ...
    it("should pick up the license key", function () {
      idempotentEnv('ONEAPM_LICENSE_KEY', 'hambulance', function (tc) {
    ...
    it("should pick up the collector host", function () {
      idempotentEnv('ONEAPM_HOST', 'localhost', function (tc) {
    ...
    it("should pick up the collector port", function () {
      idempotentEnv('ONEAPM_PORT', 7777, function (tc) {
    ...
    it("should pick up the proxy host", function () {
      idempotentEnv('ONEAPM_PROXY_HOST', 'proxyhost', function (tc) {
    ...
    it("should pick up the proxy port", function () {
      idempotentEnv('ONEAPM_PROXY_PORT', 7777, function (tc) {
    ...
    it("should pick up the log level", function () {
      idempotentEnv('ONEAPM_LOG_LEVEL', 'XXNOEXIST', function (tc) {
    ...
    it("should pick up the log filepath", function () {
      idempotentEnv('ONEAPM_LOG', '/highway/to/the/danger/zone', function (tc) {
    ...
    it("should pick up whether server-side config is enabled", function () {
      idempotentEnv('ONEAPM_IGNORE_SERVER_CONFIGURATION', 'yeah', function (tc) {
    ...
    it("should pick up whether the agent is enabled", function () {
      idempotentEnv('ONEAPM_ENABLED', 0, function (tc) {
    ...
    it("should pick up whether the apdexT is set", function () {
      idempotentEnv('ONEAPM_APDEX', 0.666, function (tc) {
    ...
    it("should pick up whether to capture request parameters", function () {
      idempotentEnv('ONEAPM_CAPTURE_PARAMS', 'yes', function (tc) {
    ...
    it("should pick up ignored request parameters", function () {
      idempotentEnv('ONEAPM_IGNORED_PARAMS', 'one,two,three', function (tc) {
    ...
    it("should pick up whether the error collector is enabled", function () {
      idempotentEnv('ONEAPM_ERROR_COLLECTOR_ENABLED', 'NO', function (tc) {
    ...
    it("should pick up which status codes are ignored", function () {
      idempotentEnv('ONEAPM_ERROR_COLLECTOR_IGNORE_ERROR_CODES',
    ...
    it("should pick up whether the transaction tracer is enabled", function () {
      idempotentEnv('ONEAPM_TRACER_ENABLED', false, function (tc) {
    ...
    it("should pick up the transaction trace threshold", function () {
      idempotentEnv('ONEAPM_TRACER_THRESHOLD', 0.02, function (tc) {
    ...
    it("should pick up the transaction trace Top N scale", function () {
      idempotentEnv('ONEAPM_TRACER_TOP_N', 5, function (tc) {
    ...
    it("should pick up whether internal metrics are enabled", function () {
      idempotentEnv('ONEAPM_DEBUG_METRICS', true, function (tc) {
    ...
    it("should pick up whether tracing of the transaction tracer is enabled",
       function () {
      idempotentEnv('ONEAPM_DEBUG_TRACER', 'yup', function (tc) {
    ...
    it("should pick up whether URL backstop has been turned off",
       function () {
      idempotentEnv('ONEAPM_ENFORCE_BACKSTOP', 'f', function (tc) {
  ...
  describe("with default properties", function () {
    ...
    before(function () {
      ...
      delete configuration.oneapm_home;
    });
    ...
    it("should connect to the collector at tpm.oneapm.com", function () {
      expect(configuration.host).equal('tpm.oneapm.com');
    });
    ...
    it("should have a log filepath of process.cwd + oneapm_agent.log", function () {
      var logPath = path.join(process.cwd(), 'oneapm_agent.log');
  ...
  describe("when overriding the config file location via NR_HOME", function () {
    var origHome
      ...
      , CONFIGPATH = path.join(DESTDIR, 'oneapm.js')
      ;

    beforeEach(function (done) {
      if (process.env.ONEAPM_HOME) {
        origHome = process.env.ONEAPM_HOME;
      }
      ...
      fs.mkdir(DESTDIR, function (error) {
        ...
        fs.mkdir(NOPLACEDIR, function (error) {
          ...
          process.env.ONEAPM_HOME = DESTDIR;
    ...
    afterEach(function (done) {
      if (origHome) {
        process.env.ONEAPM_HOME = origHome;
      }
      else {
        delete process.env.ONEAPM_HOME;
      }
      origHome = null;
    ...
    it("should export the home directory on the resulting object", function () {
      ...
      expect(configuration.oneapm_home).equal(DESTDIR);
    });

    it("should ignore the configuration file completely when so directed", function () {
      process.env.ONEAPM_NO_CONFIG_FILE = 'true';
      process.env.ONEAPM_HOME = '/xxxnoexist/nofile';
  ...
  describe("when receiving server-side configuration", function () {
    ...
    it("shouldn't blow up when beacon is received", function () {
      expect(function () {
        config.onConnect({'beacon' : 'beacon-0.oneapm.com'});
      }).not.throws();
    });
    ...
    it("shouldn't blow up when episodes_file is received", function () {
      expect(function () {
        config.onConnect({'episodes_file' : 'js-agent.oneapm.com/nr-100.js'});
      }).not.throws();
    });

    it("shouldn't blow up when episodes_url is received", function () {
      expect(function () {
        config.onConnect({'episodes_url' : 'https://js-agent.oneapm.com/nr-100.js'});
      }).not.throws();
    });
