...
describe("RemoteMethod", function () {
  ...
  describe("when posting to collector", function () {
    ...
    describe("and parsing response", function () {
      describe("that indicated success", function () {
        var getRedirectHost
          , response = {
              return_value : 'tpm-42.oneapm.com'
            }
          ;
        ...
        it("should find the expected value", function (done) {
          method.invoke(undefined, function (error, host) {
            expect(host).equal('tpm-42.oneapm.com');
      ...
      describe("that indicated a OneAPM error", function () {
        var metricData;
        var response = {
          exception : {
        ...
        it("should pass along the OneAPM error type", function (done) {
          method.invoke([], function (error) {
            expect(error.class).equal('OneAPM::Agent::ForceRestartException');
  ...
  describe("when generating headers for a plain request", function () {
    ...
    it("should have the host from the configuration", function () {
      expect(headers.Host).equal("tpm.oneapm.com");
    });
  ...
  describe("when generating a request URL", function () {
    ...
    it("should start with the (old-style) path", function () {
      expect(parsed.pathname.indexOf('/tpm/agent.do')).equal(0);
    });
    ...
    describe("when proxy is configured", function () {
      it("should attach proxy host and port during URL canonicalization", function () {
        ...
        var expected = 'http://tpm.oneapm.com:80' +
                       '/tpm/agent.do' +
                       '?marshal_format=json&protocol_version=12&' +
      ...
      it("should proxyify host when proxy settings are complete", function () {
        ...
        var method = new RemoteMethod(TEST_METHOD, config);
        parsed = reconstitute(method._path());
        expect(parsed.hostname).equal('tpm.oneapm.com');
      });
  ...
  describe("when generating the User-Agent string", function () {
    ...
    it("should clearly indicate it's OneAPM for Node", function () {
      expect(ua).include('OneAPM-NodeAgent');
    });
